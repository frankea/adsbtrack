...
**Window and tie-break caveats.** Both `aligned_seconds` and `pattern_cycles` are computed from `FlightMetrics.recent_points`, a 240-sample deque that holds only the tail of the flight. At 1-2 s trace spacing that's roughly 4-8 minutes of coverage; very long pattern sessions (30+ minutes) may under-count earlier laps because their samples were evicted before extraction finalized the flight. When two runway ends produce alignment segments of exactly equal `duration_secs`, the earlier-starting segment wins (deterministic tie-break on `first_ts`); this is a behavior change from the initial ILS alignment milestone, where iteration order of runway rows decided the tie.
