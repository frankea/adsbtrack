...
**Browser-shaped request headers.** The readsb globes sit behind Cloudflare bot rules that answer bare clients with an HTML 403 (observed on globe.airplanes.live and globe.adsb.fi, Sep 2026), so `_build_headers` presents a full Chromium fingerprint: `sec-ch-ua` / `sec-fetch-*`, a same-origin `referer` carrying the hex, and `x-requested-with`. Two constraints follow. Every token in `accept-encoding` must have an httpx decoder behind it, because httpx passes an unknown `content-encoding` through raw and `_parse_trace_body` would then choke on the compressed bytes; the `httpx[brotli,zstd]` extras in `pyproject.toml` cover `br` and `zstd`, and `test_build_headers_only_advertises_encodings_httpx_can_decode` pins the header to httpx's decoder registry. And the fingerprint is exactly what these operators hunt, so keep per-source rates conservative and expect a source to vanish behind a 403 one day - the circuit breaker above and the `--source all` health skip make that quiet rather than fatal. airplanes.live in particular closed its public REST API (`api.airplanes.live`, now a 403 with a contact-us body) to anonymous callers in August 2026; adsbtrack never used that API, only the `globe_history` archive path, which still served 2023 and 2026 traces at the time of writing.
