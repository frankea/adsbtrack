...
| extractor_version | INTEGER | `adsbtrack.parser.EXTRACTOR_VERSION` that produced this row, bumped on any behavior change to extraction/derivation. NULL for legacy rows written before this column existed. |
| v2_sample_count | INTEGER | Count of this flight's trace points carrying DO-260B version-2 aircraft state (issue #30). Same counter the flight-scoped spoof gate reads (`FlightMetrics.v2_samples`). NULL for rows extracted before the column shipped. |
| integrity_degraded_pct | REAL | Share (percent, 2 dp) of the flight's v2 samples reporting `sil=0` -- the spoof gate's own tier metric surfaced per kept flight. NULL when `v2_sample_count` is 0 or for legacy rows. See docs/features.md "Integrity/jamming surface columns". |
| max_implied_speed_kt | REAL | Peak inter-fix implied ground speed (kt, 1 dp): consecutive-fix great-circle distance / elapsed time, skipping sub-10s spacing (`FlightMetrics.max_implied_speed_kt`, the issue #22 teleport detector). NULL when no qualifying fix pair or for legacy rows. |
| integrity_flagged | INTEGER | 1 when `integrity_degraded_pct >= Config.integrity_flag_degraded_pct` (5%) with >= `spoof_min_v2_samples` v2 samples, OR `max_implied_speed_kt > Config.integrity_flag_teleport_kt` (800 kt) with corroborating degradation (`integrity_degraded_pct >= Config.integrity_flag_teleport_min_degraded_pct`, 2%, same v2 floor); 0 otherwise. Standalone teleports do not flag -- they are dominated by position-decode garbage in historical traces (see docs/features.md). Marks kept-but-degraded flights (jammed-corridor traffic); outright fabrications are quarantined to `spoofed_broadcasts` and never appear here. NULL for legacy rows. |
