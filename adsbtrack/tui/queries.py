...
def count_trace_bytes(db: Database) -> int:
    """Total *stored* size of all ``trace_days`` trace payloads.
    ...
    This deliberately does not decode anything. ``trace_json`` holds a
    zlib BLOB for rows written by this version and raw JSON TEXT for
    rows predating the compression change (``db.decode_trace_json``
    sniffs between the two; rows are never migrated in place). SQLite's
    ``length()`` returns byte count for a BLOB and character count for
    TEXT, so the sum is the on-disk footprint in both cases -- which is
    what the strip reports. It is NOT the uncompressed JSON volume, and
    the number will drop for an aircraft once its rows are compressed.
    """
